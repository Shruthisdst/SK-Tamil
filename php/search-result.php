...
elseif($text!='')
{
    ...
    $query="SELECT * FROM
            ...
            WHERE year between $year1 and $year2 ORDER BY volume, issue, cur_page";
}
...
if ($num_results > 0)
{
    echo '<h1 class="clr1 gapBelowSmall">முடிவுகள் : ' . $num_results . '</h1>';
}
...
if($num_rows > 0)
{
    while($row = $result->fetch_assoc())
    {
        ...
        if ((strcmp($id, $row['titleid'])) != 0) {
            ...
            echo '  </div>';
            echo '  <span class="aTitle"><a target="_blank" href="../Volumes/' . $row['volume'] . '/' . $row['issue'] . '/index.djvu?djvuopts&amp;page=' . $row['page'] . '.djvu&amp;zoom=page">' . $row['title'] . '</a></span>';
            if($row['authid'] != 0) {
            ...
            if($text != '')
            {
                ...
                echo '<span class="aIssue"><a href="../Volumes/' . $row['volume'] . '/' . $row['issue'] . '/index.djvu?djvuopts&amp;page=' . $row['cur_page'] . '.djvu&amp;zoom=page&amp;find=' . $dtext . '/r" target="_blank">' . intval($row['cur_page']) . '</a> </span>';
            }
            $id = $row['titleid'];
        }
        else {
