    ...
    while($row = $result->fetch_assoc())
    {
        ...
        if ((strcmp($id, $row['titleid'])) != 0) {
            ...
            if($text != '')
            {
                ...
                echo '<span class="aIssue"><a href="../Volumes/djvu/' . $row['volume'] . '/' . $row['issue'] . '/index.djvu?djvuopts&amp;page=' . $row['cur_page'] . '.djvu&amp;zoom=page&amp;find=' . $dtext . '/r" target="_blank">' . intval($row['cur_page']) . '</a> </span>';
            }
            $id = $row['titleid'];
        }
        else {
